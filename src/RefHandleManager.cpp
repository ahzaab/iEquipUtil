...
bool RefHandleManager::Load([[maybe_unused]] SKSE::SerializationInterface* a_intfc, [[maybe_unused]] uint32_t a_version)
{
    Locker locker(_lock);
    ...
    if (!a_intfc->ReadRecordData(numEntries)) {
        logger::error("Failed to load handle count!\n"sv);
        return false;
    }
...
bool RefHandleManager::InvalidateAndDispatch(const RE::TESForm* a_item, UniqueID a_uniqueID)
{
    ...
    // this is done later in OnRefHandleInvalidated
    ...
    -> std::optional<EntryData>
{
    ...
    if (!object) {
        logger::error("Form is not a bound object!\n"sv);
        return std::nullopt;
    }

    if (a_handle > kLargestHandle || a_handle == kInvalidRefHandle) {
        logger::error("Ref handle is invalid!\n"sv);
        return std::nullopt;
    }
    ...
    if (idIt == _handleToIDMap.end()) {
        logger::error("Handle not found in map!\n"sv);
        return std::nullopt;
    }
    ...
    -> RefHandle
{
    ...
    logger::critical("Ran out of ref handles!\n"sv);
