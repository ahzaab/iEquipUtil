...
namespace SoulSeeker
{
    namespace
    {
        ...
        const SoulGem& NearestNeighbour(const GemList& a_gems, const SoulGem& a_comp)
        {
            static auto diffX = [](const SoulGem& a_lhs, const SoulGem& a_rhs) -> float {
                return static_cast<float>(static_cast<int32_t>(a_lhs.gemSize) - static_cast<int32_t>(a_rhs.gemSize));
            };

            static auto diffY = [](const SoulGem& a_lhs, const SoulGem& a_rhs) -> float {
                return static_cast<float>(static_cast<int32_t>(a_lhs.soulSize) - static_cast<int32_t>(a_rhs.soulSize));
            };
        ...
        bool ValidateParams(SoulLevel a_reqCharge, FillMethod a_fillMethod)
        {
            if (a_reqCharge < SoulLevel::kPetty || a_reqCharge > SoulLevel::kGrand) {
                logger::warn("Invalid soul size! ({})"sv, static_cast<int32_t>(a_reqCharge));
                return false;
            }
            ...
            case FillMethod::kUseLargestSoul:
                break;
            default:
                logger::warn("Invalid fill method! ({})"sv, static_cast<int32_t>(a_fillMethod));
                return false;
            }
        ...
        void ApplyVerticalShift(SoulLevel a_reqCharge, FillMethod a_fillMethod, GemList& a_gems)
        {
            ...
            case FillMethod::kSmallerSoulsFirst:
                for (auto& gem : a_gems) {
                    gem.soulSize = static_cast<SoulLevel>(static_cast<int32_t>(gem.soulSize) + static_cast<int32_t>(a_reqCharge));
                }
                break;
            case FillMethod::kUseLargestSoul:
                for (auto& gem : a_gems) {
                    if (gem.soulSize < a_reqCharge) {
                        gem.soulSize = static_cast<SoulLevel>(static_cast<int32_t>(a_reqCharge) - static_cast<int32_t>(gem.soulSize) + static_cast<int32_t>(SoulLevel::kGrand));
                    }
                }
                break;
            }
    ...
    int32_t BringMeASoul(VM* a_vm, StackID a_stackID, RE::StaticFunctionTag*, uint32_t a_reqCharge, uint32_t a_fillMethod, bool a_partialFill, bool a_wasteOK)
    {
        ...
        return static_cast<int32_t>(foundGem.origSoulSize);
    }
